...
def train(args: argparse.Namespace) -> None:
    ...
    if args.dataset:
        ...
        if loaded_config:
            config = SolverConfig(**{**config.to_dict(), **loaded_config})
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in loader:
            ...
            grad_penalty = 1.0e-4 * torch.mean((pred[:, :, 1:] - pred[:, :, :-1]) ** 2)
            loss = mse + grad_penalty
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a neural fast-rollout surrogate.")
