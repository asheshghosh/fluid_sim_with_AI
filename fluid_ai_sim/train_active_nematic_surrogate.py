...
from .active_nematic import ActiveNematicConfig
from .data import generate_active_nematic_trajectories, load_dataset, make_transition_pairs, save_dataset
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.dataset:
        ...
        if loaded_config:
            config = _config_from_dict(loaded_config, config)
    else:
        trajectories = generate_active_nematic_trajectories(
            config,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in loader:
            ...
            loss = mse + args.smoothness_weight * (smooth_x + smooth_y)
    ...
    solver_config = {**config.to_dict(), "equation": "active_nematic_beris_edwards_2d_periodic"}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a surrogate for 2D periodic active nematic hydrodynamics.")
    ...
    parser.add_argument("--checkpoint", default="runs/active_nematic_fno.pt", help="Output checkpoint path.")
